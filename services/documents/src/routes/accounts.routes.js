  ...
  async (req, res, next) => {
    try {
      ...
      if (Object.prototype.hasOwnProperty.call(req.body, 'settings')) {
        ...
        params.settingsJson = JSON.stringify(req.body.settings || {});
      }
  ...
  async (req, res, next) => {
    try {
      ...
      return R.accepted(res, {
        ...
        status: account.provider === 'manual' ? 'manual_provider_ready' : 'connector_pending',
        message: account.provider === 'manual'
          ? 'La cuenta manual no sincroniza remoto; usar importación manual para poblar la bandeja.'
          : 'Conector registrado. La sincronización remota queda para la siguiente etapa.',
      });
    } catch (err) { next(err); }
  }
);
