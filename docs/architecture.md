# Arquitectura

## Resumen

VetManager Pro usa un `gateway` Node/Express delante de microservicios Node. El gateway valida auth, aplica controles de seguridad, inyecta contexto de usuario y proxyea a los servicios internos con firma HMAC.

## Diagrama

```mermaid
flowchart LR
  FE[andromeda-front]
  GW[gateway]
  AU[auth]
  PA[patients]
  ME[medical]
  BI[billing]
  NO[notifications]
  RE[reports]
  AI[ai]
  DB[(MySQL)]
  RR[(MySQL Read Replica)]
  RD[(Redis)]
  EXT[Stripe / MercadoPago / OpenAI / FCM / Twilio]

  FE --> GW
  GW --> AU
  GW --> PA
  GW --> ME
  GW --> BI
  GW --> NO
  GW --> RE
  GW --> AI

  AU --> DB
  PA --> DB
  ME --> DB
  BI --> DB
  NO --> DB
  RE --> DB
  RE --> RR
  AI --> DB

  GW --> RD
  AU --> RD
  NO --> RD

  BI --> EXT
  AI --> EXT
  NO --> EXT
```

## Decisiones actuales

- Escrituras van al primario MySQL.
- Lecturas pesadas de reportes pueden ir a `MYSQL_READ_*`.
- Redis se usa para revocación de JWT, rate limiting, RBAC cache y pub/sub.
- Notificaciones fallidas se encolan en `notification_retry_jobs`.
- IA usa circuit breaker para no bloquear toda la consulta cuando falla el proveedor.

## Riesgos todavía abiertos

- Sin broker/event bus dedicado.
- Sin service discovery real.
- Frontend sigue en JavaScript sin TypeScript.
- No hay E2E frontend todavía.
