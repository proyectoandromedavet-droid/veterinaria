...
/**
 * Integration tests — Usage limits middleware
 *
 * Tests requireUsageLimit() as Express middleware with mocked Redis.
 */

// Mock Redis before requiring usageLimits
jest.mock('redis', () => {
  const store = new Map();
  const client = {
...
function buildApp(feature) {
  ...
  app.use((req, _res, next) => {
    req.user = { orgId: parseInt(req.headers['x-org-id'] || '1') };
    next();
  });
...
afterEach(() => { redis._reset(); jest.clearAllMocks(); });

describe('requireUsageLimit middleware', () => {
  test('200 — allows request when under limit', async () => {
    // plan=free, ai_diagnosis limit=5, current usage=0
  ...
  test('429 — blocks request when over limit', async () => {
    redis._store.set('org:plan:1', 'free');
    // Set current month usage to 5 (= limit)
  ...
  test('200 — enterprise plan has unlimited usage', async () => {
    ...
    const month = new Date().toISOString().slice(0, 7);
    // Even with high usage, enterprise is unlimited
  ...
  test('200 — fails open when no orgId (unauthenticated)', async () => {
    const app = express();
    // No user middleware
  ...
  test('200 — fails open on Redis error', async () => {
